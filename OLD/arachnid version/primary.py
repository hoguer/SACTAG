import tfidf
import math
from helper_functions import rpartition, file_dict, print_to_csv_file, read_ngram_list_from_file
...
class tfidf_compare:
    ...
    def __init__(self, document1, document2, subcorpora, num_grams, variant_ngrams = False, spread = None, stopwords = True):
        ...
        print "current test: num_grams: ", self.num_grams, ", spread: ", self.spread, ", stopwords: ", self.stopwords, ", stopword file: ", self.stopword_file, ", subcorpora: ", self.subcorpora, ", auth1: ", self.doc1, ", auth2: ", self.doc2, ", variant definition: ", self.vary_defn
                        

    def compare(self):
        
	print "starting compare"
	compare_file =self.dir + str(self.num_grams) + "/" + self.subcorpora + "_" + str(self.spread) + "_" + "v" + str(self.vary_defn) + "_" + "sw" + str(self.stopwords) + ".txt"
        if os.path.exists(compare_file):
            ...
            if os.path.exists(self.doc1_tfidf_file):
                ...
                doc1dict = self.read_ngram_dict_from_file(self.doc1_tfidf_file)
            else:
                print "doc1 file does not exist yet. Creating tfidf score file for doc1."
                doc1dict = self.tf_idf(self.doc1)
            if os.path.exists(self.doc2_tfidf_file):
                print "doc2 file already exists. Reading data from file"
                doc2dict = self.read_ngram_dict_from_file(self.doc2_tfidf_file)
            else:
                print "doc2 file does not exist yet. Creating tfidf score file for doc2."
                doc2dict = self.tf_idf(self.doc2)

            self.master = self.generate_list_of_shared_ngrams_with_tfidf_scores(doc1dict, doc2dict)
    ...
    def generate_list_of_shared_ngrams_with_tfidf_scores(self,doc1_tfidf, doc2_tfidf):
        ...
        document1 = []
        document2 = []
        print "generating list of shared ngrams with tfidf_scores"

        #make a list of ngrams without their tfidf scores
        doc1_ngrams = doc1_tfidf.keys()
        doc2_ngrams = doc2_tfidf.keys()
        shared_ngrams = list(set(doc1_ngrams).intersection(doc2_ngrams))                

        #make list of lists with [ngram, score1, score2]
        c = 0
        print "len(shared_ngrams): ", len(shared_ngrams)
        for item in shared_ngrams:
            document1.append(float(doc1_tfidf[item]))
            document2.append(float(doc2_tfidf[item]))
	    self.amt_done(c, len(doc1_tfidf))
	output_list = [shared_ngrams, document1, document2]
        self.shared_scores_to_file(output_list)
        return output_list

    def amt_done(self, currItems, total):
	if (currItems % 100) == 0:
	    f = open("where_in_compare.txt", "w")
	    f.write("has computed " + str(currItems) + " out of " + str(total) + "\n clock: " + str(time.clock()))
	    f.close()

    def shared_scores_to_file(self, combo_list):
        print "in shared_scores_to_file"
        combo_list = self.sort_scores(combo_list)
        toFile = ""
        for i in range(len(combo_list[0])):
            toFile += self.list_to_string_w_sep(combo_list[0][i], "\t") + "\t" +  str(combo_list[1][i]) + "\t" + str(combo_list[2][i]) + "\n"
        f = open(self.dir + str(self.num_grams) + "/" + self.subcorpora + "_" + str(self.spread) + "_" + "v" + str(self.vary_defn) + "_" + "sw" + str(self.stopwords) + ".txt", "w")
        f.write(toFile)
        f.close()

    def list_to_string_w_sep(self,alist, sep):
        output = ""
        for item in alist:
            output += item + sep
        return output[:-1]
    ...
    def sort_scores(self, alist):
        ...
        combined_scores =  map(lambda x,y:self.compute_score(refpoint,x, y), alist[1], alist[2])

        print "len(combined_scores): ", len(combined_scores)
        print "sorted combined scores: ", sorted([(combined_scores[i], i) for i in range(len(combined_scores))])

        sorted_distances = sorted([(combined_scores[i], i) for i in range(len(combined_scores))])

        print "len(alist[0]): ", len(alist[0])
        print "len(alist[1]): ", len(alist[1])
        print "len(alist[2]): ", len(alist[2])
        c = 0
        for item in sorted_distances:
            i = item[1]
            ngrams.insert(c, alist[0][i])
            docu1.insert(c, alist[1][i])
            docu2.insert(c, alist[2][i])
            c+=1
        return [ngrams, docu1, docu2]
...
#        _tfidf.save_corpus_to_file(idf_path)
    ...
    def create_and_save_idf_corpus(self):
        ...
        idf_path = "../" + str(self.num_grams) + "grams/" + self.subcorpora + "_v_" + str(self.vary_defn) + "_sw_" + str(self.stopwords) + "_s_" + str(self.spread) +  ".txt"
        num_docs = self.compile_idf_dict()#in compile...() it should check that each file has been made. 
        print "finished compiling! Now let's print to file :-)"
	_tfidf = tfidf.TfIdf(self.spread, self.vary_defn, None, self.idf_dict, self.stopword_file)
...
#            print "docs: ", tfidf_instance.get_num_docs()
    ...
    def check_time(self,quarterHours_passed):
	
	currQuarterHour = int(time.clock()/15)
	if currQuarterHour > quarterHours_passed:
		print "another 15 mins gone by"
	
	return currQuarterHour
...
test1_hist =  tfidf_compare('Thucydides_Hist.', 'Herodotus_Hist.', 'Hist.', 2, True, 2)
#test1_hist.create_and_save_idf_corpus()
print "here we go with compare"
test1_hist.compare()

test2_hist =  tfidf_compare('Thucydides_Hist.', 'Herodotus_Hist.', 'Hist.', 2, False, 2)
#test2_hist.create_and_save_idf_corpus()
test2_hist.compare()

test3_hist =  tfidf_compare('Thucydides_Hist.', 'Herodotus_Hist.', 'Hist.', 2, True, 3)
#test3_hist.create_and_save_idf_corpus()
test3_hist.compare()

test4_hist =  tfidf_compare('Thucydides_Hist.', 'Herodotus_Hist.', 'Hist.', 2, False, 3)
#test4_hist.create_and_save_idf_corpus()
test4_hist.compare()

test5_hist =  tfidf_compare('Thucydides_Hist.', 'Herodotus_Hist.', 'Hist.', 3, True, 3)
#test5_hist.create_and_save_idf_corpus()
test5_hist.compare()

test6_hist =  tfidf_compare('Thucydides_Hist.', 'Herodotus_Hist.', 'Hist.', 3, False, 3)
#test6_hist.create_and_save_idf_corpus()
test6_hist.compare()

#-------------------------

test1_ap =  tfidf_compare('Aeschines_Orat.','Demosthenes_Orat.', 'Attic_Prose', 2, True, 2)
#test1_ap.create_and_save_idf_corpus()
test1_ap.compare()

test2_ap =  tfidf_compare('Aeschines_Orat.','Demosthenes_Orat.', 'Attic_Prose',2, False, 2)
#test2_ap.create_and_save_idf_corpus()
test2_ap.compare()
                          
test3_ap =  tfidf_compare('Aeschines_Orat.','Demosthenes_Orat.', 'Attic_Prose',2, True, 3)
#test3_ap.create_and_save_idf_corpus()
test3_ap.compare()

test4_ap =  tfidf_compare('Aeschines_Orat.','Demosthenes_Orat.', 'Attic_Prose',2, False, 3)
#test4_ap.create_and_save_idf_corpus()
test4_ap.compare()

test5_ap =  tfidf_compare('Aeschines_Orat.','Demosthenes_Orat.', 'Attic_Prose',3, True, 3)
#test5_ap.create_and_save_idf_corpus()
test5_ap.compare()

test6_ap =  tfidf_compare('Aeschines_Orat.','Demosthenes_Orat.', 'Attic_Prose', 3, False, 3)
#test6_ap.create_and_save_idf_corpus()
test6_ap.compare()
